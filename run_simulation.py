...
def current_series(device, min_current=None,max_current=None,n_steps=None,current_range=None,H_field=10,H_units="mT", current_units="uA",solve_time=200):
	...
	for current_value in currents:
		# Build terminal currents mapping depending on device terminals
		term_map = {}
		# support common terminal names: source1, drain1, source2, drain2
		if hasattr(device, 'terminals') and len(device.terminals) == 2:
			term_map = dict(source1=current_value, drain1=-current_value)
		else:
			term_map = dict(source1=current_value, drain1=-current_value,
						source2=current_value, drain2=-current_value)
		solution=tdgl.solve(
			device=device,
			options=options,
			applied_vector_potential=applied_vector_potential,
			terminal_currents=term_map
		)
